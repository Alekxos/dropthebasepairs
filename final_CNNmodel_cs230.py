...
def save_params(parameters, file):
    ''' saves parameters array to "file"
        file: string of file name to save that ends in ".pkl"'''
    with open(file, 'wb') as f:
        pickle.dump(parameters, f)

def load_params(file):
    '''loads and returns an array of parameters
        file: string of file name that ends in ".pkl"'''
...
def shuffle_data(X,Y):
    '''first index of both X and Y will be shuffled in unison,
        same every time with numpy seed'''
...
def compute_cost(Z3, Y):
    ...
    including collecting and incorporating regularization'''
...
def model(X_train, Y_train, X_test, Y_test, use_saved_params = False, use_metric = False, learning_rate = 0.00005,
    ...
    use_saved_params -- bool that tells model to start from scratch while training or use saved parameters
    use_metric -- bool, if the model is begin restored by saved params,
            it used the best metric or best accuracy parameters
    learning_rate -- learning rate of the optimization
    num_epochs -- number of epochs of the optimization loop
    
    Returns:
    best_test_metric -- the value of the best test metric that the model achieved while training
    best_test_accuracy -- the value of the best test accuracy that the model achieved while training
    parameters -- convolutional filter parameters learnt by the model.
                (all model parameters are save in a best metric and best accuracy tensorflow saver file)
    """
...
#here is good starter code to run in the shell
